...
define('USERNAME',"charankulal0241@gmail.com");
define('PASSWORD',"ikfvpykfxzewzrrr");
define('SEND_FROM',"charankulal0241@gmail.com");
define('REPLY_TO_NAME',"Jeevan");
...
Class Action {
	...
	function login_customer(){
		extract($_POST);
			$qry = $this->db->query("SELECT * FROM customer where email = '".$email."' and password = '".md5($password)."'  ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'password' && !is_numeric($key))
					$_SESSION['login_cust_'.$key] = $value;
	...
	function save_parcel(){
		...
		// if(isset($save) && isset($ids)){
				...
				// $mob=$_POST['sender_contact'];
				// $cust_email=$this->db->query("SELECT email FROM customer where phone_number = '$mob'");
